    ...
    public class PixelRuler : FrameworkElement
    {
        ...
        public double Length
        {
            get
            {
                if (this.AutoSize)
                {
                    ...
                    //return ActualWidth / 96d * 2.54d * 10 / Zoom;
                }
                else
                {
        ...
        protected override void OnRender(DrawingContext drawingContext)
        {
           ...
           // drawingContext.DrawRectangle(null, BorderPen, new Rect(new Point(0.0, 0.0), new Point(xDest, Height)));
            ...
            var lngmm = ActualWidth / 96d * 2.54d * 10 / Zoom; ;
            var smstmm = SmallStep / 96d * 2.54d * 10 / Zoom;
            var stepmm = Step / 96d * 2.54d * 10 / Zoom;

            for (double dUnit = 0; dUnit < lngmm; dUnit += SmallStep)
            {
                double d = dUnit * 96d / 2.54d * this.Zoom;
                ...
                else
                {
                    startHeight = Height;
                    // Main step or small step?
                    endHeight = ((dUnit % Step == 0) ? SegmentHeight : SegmentHeight * 1.5d);
                }

                drawingContext.DrawLine(ThinPen, new Point(d, startHeight), new Point(d, endHeight));

                if ((dUnit != 0.0) && (dUnit % Step == 0) && (dUnit < lngmm))
                {
